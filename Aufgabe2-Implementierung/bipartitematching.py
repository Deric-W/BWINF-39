...
def enumMaximumMatching(g):
    ...
    all_matches=[]
    set1 = set(node for node, data in g.nodes(data=True) if data["bipartite"] == 0)

    #----------------Find one matching M----------------
    match=bipartite.hopcroft_karp_matching(g, set1)
...
def enumMaximumMatching2(g):
    ...
    #----------------Find one matching----------------
    match=bipartite.hopcroft_karp_matching(g, s1)
...
def example1():
    ...
    all_matches=enumMaximumMatching2(g)
